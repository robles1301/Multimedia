...
\author{
  ...
  \IEEEauthorblockA{\textit{SISTEMI MULTIMEDIALI}}\\
  \textit{UNIVERSITA’ DEGLI STUDI DI BARI
  ALDO MORO}\\
  \and
  ...
  \IEEEauthorblockA{\textit{SISTEMI MULTIMEDIALI}}\\
  \textit{UNIVERSITA’ DEGLI STUDI DI BARI ALDO MORO}\\
}
...
\midrule
CNN (from scratch)         & \textbf{82.37\%} & 79.41\% & 96.92\% & \textbf{87.30\%} \\
MobileNetV2 + RF           & 79.33\% & 76.05\% & \textbf{97.69\%} & 85.52\% \\
MobileNetV2 Fine-tuned     & 67.15\% & 65.55\% & 100.00\% & 79.19\% \\
\bottomrule
\multicolumn{5}{l}{\small Precision and recall reported for the positive (pneumonia) class.}
\end{tabular}
\end{table}
...
class (normal) more heavily, improving recall for the
pneumonia class. Accordingly, this model achieves the
second-highest recall (97.69\%) while maintaining a
reasonable F1-score of 85.52\%. One limitation of this
...
The \textbf{fine-tuned MobileNetV2} achieves perfect recall
(100.00\%), meaning it flags every true pneumonia case;
however, this comes at the cost of a substantially higher
false-positive rate, yielding the lowest precision (65.55\%)
and the lowest overall accuracy (67.15\%) of the three
models. The two-phase training strategy—freezing the
backbone in Phase 1 and unfreezing the last 20 layers in
Phase 2 with a conservative learning rate ($10^{-5}$)—successfully
prevents catastrophic forgetting while enabling domain
adaptation, but the resulting model exhibits a strong
bias towards the pneumonia class. In a clinical screening
context where the cost of a false negative exceeds that of a
false positive, this high-recall behaviour may be clinically
advantageous despite the lower accuracy.

Overall, the \textbf{custom CNN} achieves the best balance
among all metrics, obtaining the highest accuracy (82.37\%)
and the highest F1-score (87.30\%), demonstrating that a
relatively simple architecture trained from scratch can be
competitive when recall is not the sole criterion.
...
The results reveal a clear precision-recall trade-off across
the three approaches. The custom CNN achieves the highest
accuracy (82.37\%) and F1-score (87.30\%), establishing a
strong baseline despite receiving no pre-trained knowledge.
The MobileNetV2 + Random Forest hybrid improves recall to
97.69\% by leveraging rich ImageNet representations, at the
cost of a modest reduction in precision and accuracy.
The fine-tuned MobileNetV2 attains perfect recall (100.00\%),
ensuring no true pneumonia case is missed, but its precision
drops to 65.55\% and its accuracy to 67.15\%, reflecting a
pronounced bias towards the positive class induced by the
two-phase fine-tuning procedure. In a clinical screening
context where false negatives carry the highest cost, the
high-recall behaviour of the transfer learning models is
clinically desirable, even at the expense of overall accuracy.
...
\bibitem{kaggle2018}
P. Mooney, ``Chest X-Ray Images (Pneumonia),''
